...
const DetailedView = () => {
  ...
  useEffect(() => {
    const url = `/tmdb/${type}/${id}`;
    const info = async () => {
      const data = await axios.get(url).then((result) => result.data);
      setResource(data);
      const urlkey = `/tmdb/${type}/Keywords`;
      const keywords = await axios.post(urlkey, { id: data.id }).then(result=>result.data)
      setKeywords(keywords)
      const urlcast = `/tmdb/${type}/Cast`;
      const cast = await axios.post(urlcast, { id: data.id }).then(result=>result.data);
      setCast(cast)
    };
    info();
  }, []);
